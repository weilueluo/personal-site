...
import { AnimationMixer, LoopPingPong, ShaderMaterial, Vector3 } from 'three';
...
export default function MainBall({ ...props }) {
    ...
    const { nodes, materials, animations } = useGLTF(
        '/models/snowball/snow_02_4k-transformed.glb'
    );
    const uniforms = {
        uTime: { value: 0.5 },
    ...
    return (
        <group ref={group} dispose={null} scale={8} {...props}>
            <group ref={sceneRef} name='Scene'>
                <group
                    name='Sphere_cell003_cell_cell001'
                    position={[-0.96, -0.2, 0.21]}
                />
                <group
                    name='Sphere_cell003_cell_cell002'
                    position={[-0.96, -0.19, 0.22]}
                />
                <group
                    name='Sphere_cell003_cell_cell003'
                    position={[-0.95, -0.21, 0.2]}
                />
                <group
                    name='Sphere_cell016_cell007_cell001'
                    position={[0.89, 0.38, 0.26]}
                />
                <group
                    name='Sphere_cell018_cell_cell004'
                    position={[-0.01, -0.99, 0.14]}
                />
                <group
                    name='Sphere_cell018_cell_cell005'
                    position={[0, -0.99, 0.12]}
                />
                {meshes}
            </group>
        </group>
...
function getMeshes(nodes, sharedMaterial) {
    const meshNodes = Object.entries(nodes).filter(
        (entry) => entry[1].type == 'Mesh' && entry[0] != 'Sphere'
    );
