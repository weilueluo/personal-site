...
export default function ShaderRenderer(_props: BaseCompProps<"div">) {
    ...
    useEffect(() => {
        // Avoid side effects inside a state updater function; React may invoke those during render.
        let cancelled = false;
        ...
        options
            .map(opt => TYPE_2_SOLUTION[opt])
            .filter((opt): opt is string => Boolean(opt))
            .forEach(opt => newFramework.addSolution(opt));
        ...
        newFramework
            ...
            .then(() => {
                if (cancelled) return;
                return newFramework.start(maxFrames);
            })
            .then(() => {
                if (cancelled) return;
                setProcessing(false);
