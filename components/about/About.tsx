...
function SubTitle(props) {
    return <h2 className={styles['sub-title']}>{props.children}</h2>;
}
...
function MessageResponse(props: {
    status: [[MessageStatusType, string[]], ([MessageStatusType, any]) => void];
}) {
    ...
    const displayMessage = props.status[0][1].map((ele, i) => (
        <div key={i}>
            {ele}
            <br />
        </div>
