...
import { AnimeCharacter, AnimeStaff } from '../anime';
...
function Staff(props: { staffData: AnimeStaff }) {
    ...
    const imageURLs = staffData.image
        ? [staffData.image.medium, staffData.image.large]
        : [];
...
function SidePanel() {
    ...
    useEffect(() => {
        animeData.coverImage &&
            setUrls([animeData.coverImage.medium, animeData.coverImage.large]);
    }, [animeData.coverImage]);
...
function Relation_(props: { relationData: Relation }) {
    ...
    const [relationUrl, index] = useSequentiallyLoadedImageURL(
        coverImages && [coverImages.medium, coverImages.large],
    );
    const imageStyle = mergeStyles(
        styles.image,
        index == 0 && styles.loadingImage,
...
function Characters() {
    ...
    const dataManagement = useMediaCharactersFetching(
        animeData.id,
        animeData?.characters?.edges,
        animeData?.characters?.edges ? 2 : 1,
    );
    const renderFunction = (displayData: AnimeCharacter[]) =>
...
function Staffs() {
    ...
    const dataManagement = useMediaStaffsFetching(
        animeData.id,
        animeData?.staff?.edges,
        animeData?.staff?.edges ? 2 : 1,
    );
    const renderFunction = (displayData: AnimeStaff[]) =>
