...
export async function fetchFavouritesPage(page_: number = 1): Promise<AnilistPagedData<SectionMedia>> {
    const graphqlQuery = query(page(usersFavouritesAnimeNodes(MY_USER_ID, page_)))
...
export async function fetchAnilistMediaCharacters(animeID: number, page_: number): Promise<AnilistPagedData<Character>> {
...
export async function fetchAnilistMediaStaffs(animeID: number, page_: number): Promise<AnilistPagedData<Staff>> {
