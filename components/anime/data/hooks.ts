...
import {
    fetchAnilistMediaCharacters,
    fetchAnilistMediaStaffs,
    fetchFavouritesPage,
    fetchMediaList,
    INIT_PAGE_INFO,
} from './request';
...
export function useFavouritesFetching(
    existingData: SectionMedia[] = EMPTY_ARRAY,
    startPage = 1,
) {
    return useSegmentedDataFetching(
        fetchFavouritesPage,
        existingData,
        startPage,
    );
...
export function useMediaListCurrentFetching(
    existingData: SectionMedia[] = EMPTY_ARRAY,
    startPage = 1,
) {
    return useSegmentedDataFetching(
        (page: number) => fetchMediaList(page, 'CURRENT'),
        existingData,
        startPage,
    );
}

export function useMediaListPlannedFetching(
    existingData: SectionMedia[] = EMPTY_ARRAY,
    startPage = 1,
) {
    return useSegmentedDataFetching(
        (page: number) => fetchMediaList(page, 'PLANNING'),
        existingData,
        startPage,
    );
}

export function useMediaCharactersFetching(
    animeID: number,
    existingData: Character[],
    startPage = 1,
) {
    const fetchFunction = useCallback(
        (page: number) => fetchAnilistMediaCharacters(animeID, page),
        [animeID],
    );
    return useSegmentedDataFetching(fetchFunction, existingData, startPage);
}

export function useMediaStaffsFetching(
    animeID: number,
    existingData: Staff[],
    startPage = 1,
) {
    const fetchFunction = useCallback(
        (page: number) => fetchAnilistMediaStaffs(animeID, page),
        [animeID],
    );
    return useSegmentedDataFetching(fetchFunction, existingData, startPage);
}
...
export function useSegmentedDataFetching<T extends { id: number }>(
    ...
    const fetchNext = useCallback(async () => {
        ...
        const response = await fetchFunction(
            (pageInfo && pageInfo.currentPage + 1) || startPage,
        );
