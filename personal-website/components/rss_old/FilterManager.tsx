...
import { FilterSection, FlatFeed } from './RSS';
import {
    toMonday,
    toStartOfMonth,
    toStartOfTheDay,
...
class FilterDate {
    displayName: string;
    getStartDate: () => unknown;

    private constructor(displayName: string, getStartDate: () => unknown) {
...
export function filterFlatFeeds(
    ...
    for (const filterSection of filterSections) {
        flatFeeds = NAME2FILTER_FUNCTION[filterSection.name](
            flatFeeds,
            filterSection,
        );
    }
