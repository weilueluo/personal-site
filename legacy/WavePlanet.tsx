...
function MySphere() {
    ...
    useEffect(() => {
        const geometry = geoRef.current as BufferGeometry;
        if (geometry) {
            setCount(geometry.attributes.position.count);
            setPositionClone(
                JSON.parse(JSON.stringify(geometry.attributes.position.array))
            );
            setNormalsClone(
                JSON.parse(JSON.stringify(geometry.attributes.normal.array))
            );
        }
    }, []);
    ...
    return (
        <mesh>
            <sphereBufferGeometry ref={geoRef} args={[6, 128, 128]} />
