...
import { FlatFeed, FeedsMap as Name2FeedMap } from './RSS.d';
...
export function feed2flatFeeds(feed: Parser.Output<object>, name: string) {
    const { items, ...feedWithoutItems } = feed as Parser.Output<object> & {
        items?: Parser.Item[];
    };
    const safeItems = items ?? [];
    return safeItems.map(feedItem => {
        ...
        const extras = {
            name: name,
            jsDate: date ? new Date(date) : undefined,
        };
