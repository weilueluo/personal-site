...
export default function Feed(props: { flatFeed: FlatFeed; i: number }) {
    ...
    const isOverflown = useMemo(
        () => (element: HTMLElement | null, parent: HTMLElement | null) => {
            if (!element || !parent) {
                return false;
            }
    ...
    useEffect(() => {
        const description = document.getElementById(
            `feed-description-${props.i}`,
        );
        const container = document.getElementById(
            `feed-description-container-${props.i}`,
        );
    ...
    return (
        <li key={key} className={styles['feed']}>
            ...
            <div className={styles['feed-info']}>
                ...
                <span
                    className={styles['feed-date']}
                    onClick={displayTimeOnClicked}>
                    {displayTime}
                </span>
            </div>
            ...
            <span
                ...
                className={`${styles['feed-more']} ${
                    expandActive ? styles['expand-active'] : ''
                } ${overflown ? styles['overflown'] : ''}`}
                onClick={() => expandOnClick()}>
                <p
                    id={`feed-description-${props.i}`}
                    className={`${styles['feed-description']}`}>
                    {description}
                </p>
                {/* eslint-disable-next-line */}
                {overflown && expandActive && (
                    <span className={styles['feed-expand']}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                            className={styles['feed-expand-img']}
