...
class FilterDate {
    ...
    private constructor(displayName: string, getStartDate: () => Date) {
        this.displayName = displayName;
        this.getStartDate = getStartDate;
    }

    static TODAY = new FilterDate('Today', () => toStartOfTheDay(new Date()));
    static YESTERDAY = new FilterDate('Two Days', () =>
        toYesterday(new Date()),
    );
    static THIS_WEEK = new FilterDate('This Week', () => toMonday(new Date()));
    static THIS_MONTH = new FilterDate('This Month', () =>
        toStartOfMonth(new Date()),
    );
    static THIS_YEAR = new FilterDate('This Year', () =>
        toStartOfYear(new Date()),
    );
}
...
const DATE_FILTERS = (
    Object.keys(FilterDate) as Array<keyof typeof FilterDate>
)
    .filter(key => FilterDate[key] instanceof FilterDate)
    .map(key => ({
        name: key as keyof FilterDate,
        displayName: (FilterDate[key] as FilterDate).displayName,
        active: false,
    }));
...
function filterSource(flatFeeds: FlatFeed[], opt: FilterSection): FlatFeed[] {
    ...
    const filtered = flatFeeds.filter(
        feed => feed.name && nameWanted.has(feed.name),
    );
...
function filterDate(flatFeeds: FlatFeed[], opt: FilterSection): FlatFeed[] {
    ...
    for (const filter of activeDateFilters) {
        const dateKey = filter.name as keyof typeof FilterDate;
        const startDate = (FilterDate[dateKey] as FilterDate).getStartDate();
        // console.log(startDate.toDateString());

        flatFeeds = flatFeeds.filter(
            feed =>
                feed.jsDate && feed.jsDate.getTime() - startDate.getTime() >= 0,
        );
    }
...
export function filterFlatFeeds(
    flatFeeds: FlatFeed[],
    filterSections: FilterSection[],
) {
    for (const filterSection of filterSections) {
        const filterFn =
            NAME2FILTER_FUNCTION[
                filterSection.name as keyof typeof NAME2FILTER_FUNCTION
            ];
        if (!filterFn) {
            continue;
        }
        flatFeeds = filterFn(flatFeeds, filterSection);
    }
    // console.log('filtered');
    // console.log(flatFeeds);
...
export function setAllFiltersInplace(
    value: boolean,
...
export function isAllFiltersSame(section: FilterSection) {
    let firstFoundActive: boolean | null = null;
