...
import {
    ...
    useMediaCharactersFetching,
    useMediaStaffsFetching,
} from '../anime/data/hooks';
import { useSequentiallyLoadedImageURL } from '../anime/data/misc';
import type { Character as CharacterData, Media, Relation, Staff as StaffData } from '../anime/data/query';
...
function BannerImage() {
    ...
    const [imageUrl, index] = useSequentiallyLoadedImageURL(
        animeData?.bannerImage ? [animeData.bannerImage] : []
    );

    const alt = animeData.title
        ? animeData.title.english
        : animeData.id.toString();

    const imageStyle = mergeStyles(
        styles.bannerImage,
        index == 0 ? styles.loadingImage : ''
    );
...
function VoiceActorName(props: { characterData: CharacterData }) {
    const VAs = props.characterData.voiceActors;
    const [VANames, setVANames] = useState<string[]>([]);
    const [VANAme, nextName] = useRotateString(VANames);
    useEffect(() => {
        if (VAs && VAs.length > 0) {
            const names = VAs[0].name;
            if (names) {
                const nextNames = [names.full, names.native, ...(names.alternative || [])]
                    .filter(Boolean) as string[];
                setVANames(nextNames);
            } else {
                setVANames([]);
            }
        }
...
function Character(props: { characterData: CharacterData }) {
    const charData = props.characterData;
    const charNode = charData.node;

    const urls = charNode?.image
        ? [charNode.image.medium, charNode.image.large].filter(Boolean) as string[]
        : [];
    const [url, index] = useSequentiallyLoadedImageURL(urls);
    const alt = charNode?.name ? charNode.name.full : 'Character Image';
    const charNames = charNode?.name
        ? [charNode.name.full, charNode.name.native, ...(charNode.name.alternative || [])]
              .filter(Boolean) as string[]
        : [];
    const [charName, nextCharName] = useRotateString(charNames);
    const href = charNode?.siteUrl;
    ...
    return (
        <li className={styles.card}>
            ...
            <div className={cardTitleStyle}>
                <strong>{charData.role} </strong>
                <br />
                <span onClick={nextCharName}>{charName}</span>
                <br />
...
function Staff(props: { staffData: StaffData }) {
    const staffData = props.staffData.node;

    const staffNames = staffData?.name
        ? [
              staffData.name.full,
              staffData.name.native,
              ...(staffData.name.alternative || []),
          ].filter(Boolean) as string[]
        : [];
    ...
    const imageURLs = staffData?.image
        ? [staffData.image.medium, staffData.image.large].filter(Boolean) as string[]
        : [];
...
const SMALL_TEXT_LOADING_CARDS = Array.from(Array(10)).map((_, i) => (
    <SmallTextLoadingCard key={i} />
));

const SECTION_LOADING_CARDS = Array.from(Array(10)).map((_, i) => (
    <LoadingCard
        key={i}
        style={{
            height: 'clamp(140px, 200px, 35vw)',
            width: 'clamp(80px, 120px, 20vw)',
        }}
    />
));
...
function NextAiring() {
    ...
    const timeUntilNextAiring =
        ...
        timeSinceSeconds(animeData.nextAiringEpisode?.timeUntilAiring || 0);
...
function SidePanel() {
    ...
    useEffect(() => {
        if (animeData.coverImage) {
            const nextUrls = [animeData.coverImage.medium, animeData.coverImage.large].filter(Boolean) as string[];
            setUrls(nextUrls);
        }
...
function AnimeTitle() {
    ...
    useEffect(() => {
        const titles = animeData && animeData.title
            ? [
                  animeData.title.romaji,
                  animeData.title.native,
                  animeData.title.english,
                  ...(animeData.synonyms ? animeData.synonyms : []),
              ].filter(Boolean) as string[]
            : ['N/A'];
        const safeTitles = titles.length > 0 ? titles : ['N/A'];
        setTitle(safeTitles[index % safeTitles.length]);
    }, [index, animeData]);

    return (
        <span className={styles.animeTitle} onClick={nextTitle}>
            {title}
        </span>
    );
}
...
function AnimeDescription() {
    ...
    const descStyle = mergeStyles(
        styles.animeDescription,
        !animeData.description ? styles.noDescriptionAvailable : '',
    );
...
function Relation_(props: { relationData: Relation }) {
    ...
    const node = relationData.node;
    const titles = node?.title;
    const coverImages = node?.coverImage;

    const [relationUrl, index] = useSequentiallyLoadedImageURL(
        coverImages ? [coverImages.medium, coverImages.large].filter(Boolean) as string[] : [],
    );
    const imageStyle = mergeStyles(
        styles.image,
        index == 0 ? styles.loadingImage : '',
    ...
    const [title, nextTitle] = useRotateString(
        titles
            ? [titles.english, titles.native, titles.romaji].filter(Boolean) as string[]
            : []
    );
    const href = node?.siteUrl;
    ...
    return (
        <li className={styles.card}>
            ...
            <div className={styles.title}>
                <strong>{relationData.relationType}</strong>
                <br />
...
function Relations() {
    ...
    const relations = animeData?.relations?.edges ? (
        animeData.relations.edges.map(relationData => (
            <Relation_ key={relationData.id} relationData={relationData} />
        ))
    ...
    return (
        <div className={styles.relationContainer}>
            ...
            <ul className={cardListStyle}>{relations}</ul>
        </div>
    );
}
...
function CardsSection<T>({
    ...
    renderFunction: (data: T[]) => ReactElement[];
}) {
    ...
    useEffect(() => {
        if (
            !firstClick &&
            displayAmount > loadedData.length &&
    ...
    const buttonStyle = mergeStyles(
        styles.loadButton,
        displayAmount > loadedData.length && !pageInfo.hasNextPage
            ? styles.allLoaded
            : '',
    ...
    return (
        <div className={styles.sectionContainer}>
            <div className={styles.sectionHeader}>
                ...
                {animeData.id != LOADING_ID && (
                    <button className={buttonStyle} onClick={loadButtonClick}>
                        {loadButtonText}
                    </button>
                )}
            </div>
...
function Characters() {
    ...
    const dataManagement = useMediaCharactersFetching(
        animeData.id,
        characterEdges,
        characterEdges.length > 0 ? 2 : 1,
    );
    const renderFunction = (displayData: CharacterData[]) =>
        displayData.map(charData => (
            <Character key={charData.id} characterData={charData} />
        ));
    return (
        <CardsSection
            title="Characters"
            segmentedDataFetching={dataManagement}
...
function Staffs() {
    const animeData = useContext(AnimeDataContext);
    const staffEdges = animeData?.staff?.edges ?? [];
    const dataManagement = useMediaStaffsFetching(
        animeData.id,
        staffEdges,
        staffEdges.length > 0 ? 2 : 1,
    );
    const renderFunction = (displayData: StaffData[]) =>
        displayData.map(staffData => (
...
const TRAILER_LOADING_CARD = (
    <LoadingCard banner={true} style={{ height: '350px' }} scale={6} />
);
...
export default function AnimeDetails(props: { animeID: string }) {
    ...
    useEffect(() => {
        fetchAnilistMedia(Number(props.animeID)).then(data =>
            setAnimeData(data),
        );
    }, [props.animeID]);
