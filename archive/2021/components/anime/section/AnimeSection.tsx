...
function AnimeCard({ animeData }: { animeData: FavAnimeMedia }) {
    ...
    const titles = animeData.title
        ? [animeData.title.english, animeData.title.romaji, animeData.title.native].filter(Boolean) as string[]
        : EMPTY_ARRAY;
...
export default function AnimeSection<T extends SectionAnimeMedia>(props: {
    ...
    const [displayAnimeDataList, setDisplayAnimeDataList] = useState<T[]>([]);
    useEffect(() => {
