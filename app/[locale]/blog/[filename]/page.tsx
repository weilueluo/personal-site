...
export default async function Page({ params }: { params: BlogParams | Promise<BlogParams> }) {
    const { locale, filename } = await params;
    const blogContentPromise = fetchBlogContent(filename);
    const blogCommitPromise = fetchBlogCommit(filename);
